...
struct mempool* memory_allocate_mempool(struct device* dev, uint32_t num_entries, uint32_t entry_size) {
	...
	if ((dev -> cfd == -1) && HUGE_PAGE_SIZE % entry_size) {
		error("entry size must be a divisor of the huge page size (%d)", HUGE_PAGE_SIZE); //条目大小必须是巨大页面大小的除数
...
pthread_cond_t cond;
struct pkt_buf* refull_bufs[3010];
